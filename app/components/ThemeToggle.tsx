  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 px-3 py-2 rounded-md border transition-colors duration-200 select-none outline-none focus:outline-none focus:ring-0 ${
        isDark
          ? 'bg-dark-bg-tertiary text-light-text-primary border-[#334155] hover:bg-dark-bg-card'
    ...
    >
      {isDark ? (
        <>
          ...
          <span className="text-sm font-medium text-[#A5B4FC]">Dark</span>
        </>
      ) : (
        <>
          ...
          <span className="text-sm font-medium text-light-text-primary">Light</span>
        </>
      )}
    </button>
  );
