...
export default function MovieCard({ movie, detail }: MovieCardProps) {
  ...
  // Если нет постера, не показываем карточку
  if (!hasValidPoster) {
    return null;
  }
  ...
  const getRatingColor = (rating: string) => {
    ...
    if (numRating >= 8.0) return 'text-green-400';
    if (numRating >= 7.0) return 'text-yellow-400';
    if (numRating >= 6.0) return 'text-orange-400';
    return 'text-red-400';
  };

  return (
    <div className="group relative">
      {/* Основная карточка в стиле Netflix */}
      <div className="relative aspect-[2/3] bg-gray-900 rounded-lg overflow-hidden cursor-pointer transition-all duration-300 ease-out group-hover:scale-105 group-hover:z-50">
        <img
          src={movie.Poster}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
          ...
          {detail && detail.imdbRating && detail.imdbRating !== 'N/A' && (
            <div className="flex items-center gap-1 mb-3">
              ...
              <span className={`font-semibold ${getRatingColor(detail.imdbRating)}`}>
                {detail.imdbRating}
              </span>
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            <a
              ...
              className="bg-gray-600/80 hover:bg-gray-500/80 text-white p-2 rounded transition-colors"
      ...
      {showDetails && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-gray-800 text-white p-4 rounded-lg shadow-2xl z-50 min-w-[300px] animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  {detail.Runtime && detail.Runtime !== 'N/A' && (
                    <div>
                      ...
                      <p className="text-white font-medium">{detail.Runtime}</p>
                    </div>
                  )}
                  ...
                  {detail.Genre && detail.Genre !== 'N/A' && (
                    <div>
                      ...
                      <p className="text-white font-medium">{detail.Genre.split(', ').slice(0, 2).join(', ')}</p>
                    </div>
                  )}
                  
                  {detail.Director && detail.Director !== 'N/A' && (
                    <div>
                      <span className="text-gray-400">Режиссёр:</span>
                  ...
                  {detail.Released && detail.Released !== 'N/A' && (
                    <div>
                      <span className="text-gray-400">Выпуск:</span>
                      <p className="text-white font-medium">{detail.Released}</p>
                    </div>
                  )}
                </div>
                ...
                {detail.Actors && detail.Actors !== 'N/A' && (
                  <div>
                    ...
                    <p className="text-white text-sm font-medium mt-1">
                      {detail.Actors.split(', ').slice(0, 3).join(', ')}
                    </p>
                  </div>
                )}
              </>
